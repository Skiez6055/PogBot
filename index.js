...
client.on('message', msg=>{
  if(msg.content === ".pog butter"){
    msg.reply('RYAN RYAN. DONT LET ALMERD BITE. SUCK MY COCK IN SIEGE UWU OWO TUGGLE GOO! https://tenor.com/view/cock-penis-balls-running-gif-13812460')
  }
})
...
client.on('message', message => {
  ...
  client.on('message', message => {
    if (!message.guild) return;
    if (message.content.startsWith('.pog divorce')) {
      const user = message.mentions.users.first();
      if (user) {
        message.reply(`OHH WEE WEE FAGGOT. YOU AND @${user.tag} ARE DIVORCED AND SINGLE AGAIN AYE! https://thumbs.gfycat.com/AncientGiantHummingbird-size_restricted.gif`);
      const member = message.guild.member(user);
      
    } else {
      // Otherwise, if no user was mentioned
      message.reply("tag a person to un-marry!");
    }
    ...
    client.on('message', msg=>{
      if(msg.content === ".pog aleesha"){
        msg.reply('LMAFAO ALEESHA IS A CARPET LMAOOO **AYE** https://tenor.com/view/carpet-land-discount-carpet-shop-now-gif-15749957 ')
      }
    })
...
client.on('message', message => {
  ...
  client.on('message', msg=>{
    if(msg.content === ".pog log"){
      msg.reply('**Update on June 19th 2020** added .pog closet - Polo g is in a closet! WOw ')
    }
  })
  ...
  client.on('message', msg=>{
    if(msg.content === ".pog golden"){
      msg.reply('Shut the fuck up golden!!!! https://tenor.com/view/bad-shit-tyler-nigga-we-out-here-gif-4826401')
    }
  })
  ...
  client.on('message', message => {
    if (!message.guild) return;
    if (message.content.startsWith('.pog closet')) {
      const user = message.mentions.users.first();
      if (user) {
        message.reply(`AYE @${user.tag} LOOK WHATS IN UR CLOSET DUMMY DUM DUM https://thumbs.gfycat.com/FixedPoliteArchaeocete-size_restricted.gifn`);
      const member = message.guild.member(user);
      
    } else {
      // Otherwise, if no user was mentioned
      message.reply("Who's closet should Polo G go? Tag a motherfucker in the server. Fuck you");
    }
        ...
        if (member) {
          ...
          member
            ...
            .then(() => {
              ...
              message.reply(`I BANNED THE  ${user.tag} IN DA ASS ***AYE*** https://thumbs.gfycat.com/CompetentCloseItaliangreyhound-size_restricted.gif`);
            })
            .catch(err => {
              // An error happened
